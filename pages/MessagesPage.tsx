...
export const MessagesPage: React.FC<{ state: AppState, setState: any, showToast: any, embedded?: boolean }> = ({ state, setState, showToast, embedded }) => {
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className={`flex flex-col ${embedded ? 'h-full' : 'h-[calc(100vh-160px)] md:h-[calc(100vh-100px)]'}`}
    >
      ...
      <div className="mt-4 flex gap-2 bg-white/40 backdrop-blur-md p-2 rounded-2xl border border-zinc-200/50 shadow-sm">
        <label className="p-3 bg-white/60 border border-zinc-200/50 rounded-xl cursor-pointer hover:bg-white transition-all text-zinc-500 hover:text-velatra-accent shadow-sm flex items-center justify-center">
          <PlusIcon size={20} />
        ...
        <Input 
          ...
          className="!py-3 !bg-white/60 backdrop-blur-xl !border-zinc-200/50 shadow-inner focus:!bg-white" 
        />
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Button onClick={sendMessage} className="!p-3 h-full shadow-lg shadow-velatra-accent/20">
