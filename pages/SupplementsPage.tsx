    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8 page-transition pb-24">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-velatra-textMuted text-sm font-medium mt-1">Gérez vos produits et commandes</p>
        </div>
        <div className="flex gap-2 bg-white/5 p-1 rounded-xl border border-white/10">
          <button 
            onClick={() => setActiveTab('products')}
            className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${activeTab === 'products' ? 'bg-velatra-accent text-white' : 'text-velatra-textMuted hover:text-white'}`}
          >
            Produits
          </button>
          <button 
            onClick={() => setActiveTab('orders')}
            className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${activeTab === 'orders' ? 'bg-velatra-accent text-white' : 'text-velatra-textMuted hover:text-white'}`}
          >
            Commandes
          </button>
        </div>
      ...
      {activeTab === 'products' && (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Button onClick={() => setShowAddProduct(true)} variant="primary" className="!py-2">
          ...
          {showAddProduct && (
            <Card className="bg-white/5 border-white/10">
              <h3 className="text-lg font-bold text-white mb-4">Nouveau produit</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                <div>
                  <label className="block text-xs text-velatra-textMuted mb-1">Nom</label>
                  <Input value={newProduct.nom} onChange={e => setNewProduct({...newProduct, nom: e.target.value})} placeholder="Ex: Whey Isolate" />
                </div>
                <div>
                  <label className="block text-xs text-velatra-textMuted mb-1">Catégorie</label>
                  <select 
                    value={newProduct.cat} 
                    onChange={e => setNewProduct({...newProduct, cat: e.target.value})}
                    className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-velatra-accent"
                  >
                ...
                <div>
                  <label className="block text-xs text-velatra-textMuted mb-1">Prix Achat (€)</label>
                  <Input type="number" value={newProduct.prixAchat} onChange={e => setNewProduct({...newProduct, prixAchat: Number(e.target.value)})} />
                </div>
                <div>
                  <label className="block text-xs text-velatra-textMuted mb-1">Prix Vente (€)</label>
                  <Input type="number" value={newProduct.prixVente} onChange={e => setNewProduct({...newProduct, prixVente: Number(e.target.value)})} />
                </div>
                <div>
                  <label className="block text-xs text-velatra-textMuted mb-1">Stock</label>
            ...
            {state.supplementProducts.map(product => (
              <Card key={product.id} className="bg-white/5 border-white/10 flex flex-col justify-between">
                <div>
                  ...
                  <div className="text-sm text-velatra-textMuted mb-4">Stock: <span className={product.stock < 5 ? "text-red-500 font-bold" : "text-white"}>{product.stock}</span> unités</div>
                </div>
                <div className="flex justify-between items-end border-t border-white/10 pt-4 mt-2">
                  <div>
                    <div className="text-xs text-velatra-textMuted">Prix Achat</div>
                    <div className="font-mono text-white">{product.prixAchat} €</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-velatra-textMuted">Prix Vente</div>
                    <div className="font-mono text-velatra-accent font-bold text-lg">{product.prixVente} €</div>
                  </div>
                </div>
              </Card>
            ))}
            {state.supplementProducts.length === 0 && !showAddProduct && (
      ...
      {activeTab === 'orders' && (
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {state.supplementOrders.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(order => {
                ...
                <Card key={order.id} className="bg-white/5 border-white/10">
                  <div className="flex flex-col md:flex-row justify-between gap-4">
                    <div>
                      ...
                      <div className="text-xs text-velatra-textMuted mb-3">
                      ...
                      <div className="space-y-1">
                        {order.produits.map((p, i) => (
                          <div key={i} className="text-sm text-white flex gap-2">
                            <span className="text-velatra-textMuted">{p.quantite}x</span> {p.nom} <span className="text-velatra-textMuted text-xs ml-2">({p.prixUnitaire}€)</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="flex flex-col justify-between items-end min-w-[120px]">
                      <div className="text-right mb-4">
                        <div className="text-xs text-velatra-textMuted uppercase tracking-widest">Total</div>
