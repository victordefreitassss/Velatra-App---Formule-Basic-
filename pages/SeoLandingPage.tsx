...
const pages: Record<string, SeoPage> = {
  ...
  '/logiciel-studio-coaching': {
    ...
    sections: [
      { title: 'Une vue partagée de vos adhérents', text: 'Retrouvez les profils, les programmes et les informations utiles dans votre espace de gestion.' },
...
export default function SeoLandingPage() {
  ...
  const faq = [
    ...
    { question: 'Comment découvrir les fonctionnalités disponibles ?', answer: 'La page Fonctionnalités présente les outils de l’application. La page Tarifs détaille les offres disponibles.' },
  ];
  ...
  return <>
    ...
    <main className="marketing-seo-page">
      <section className="marketing-seo-hero"><div className="marketing-container"><nav className="marketing-breadcrumb" aria-label="Fil d’Ariane"><Link to="/">Accueil</Link><span>/</span><Link to="/fonctionnalites">Velatra</Link><span>/</span><span>{page.title.split(':')[0]}</span></nav><span className="marketing-kicker"><span className="marketing-kicker-dot" /> VOTRE ACTIVITÉ DE COACHING</span><h1>{page.shortTitle}</h1><p>{page.intro}</p><div className="marketing-hero-actions"><Link to="/register" className="marketing-button marketing-button-primary">Essayer Velatra gratuitement <ArrowRight size={16} /></Link><Link to="/contact" className="marketing-button marketing-button-secondary">Demander une démonstration</Link></div><div className="marketing-seo-proof"><div><b>01</b><span>Un espace pour<br />vos adhérents</span></div><div><b>02</b><span>Les outils pour<br />vos programmes</span></div><div><b>03</b><span>Une vue plus claire<br />sur votre activité</span></div></div></div></section>
      <section className="marketing-seo-content"><div className="marketing-container"><div className="marketing-seo-heading"><span className="marketing-kicker">DÉCOUVRIR VELATRA</span><h2>Les bons outils pour<br /><em>mieux accompagner.</em></h2></div><div className="marketing-seo-feature-grid">{page.sections.map((section, index) => { const Icon = iconSet[index]; return <article key={section.title}><span className="marketing-seo-icon"><Icon size={18} /></span><small>0{index + 1}</small><h3>{section.title}</h3><p>{section.text}</p></article>; })}</div><div className="marketing-seo-included"><div><span className="marketing-kicker">DANS VOTRE ESPACE</span><h2>Votre quotidien de coach,<br /><em>rassemblé.</em></h2><p>{page.intro}</p><Link to="/fonctionnalites" className="marketing-text-link">Voir les fonctionnalités <ArrowRight size={15} /></Link></div><ul>{page.features.map((feature) => <li key={feature}><Check size={16} />{feature}</li>)}</ul></div></div></section>
