  ...
  return <>
    ...
    <main className="marketing-blog">
      ...
      <section className="marketing-container marketing-blog-content">
        ...
        <div className="marketing-blog-grid">{visiblePosts.map((post) => <article key={post.slug} className="marketing-blog-card"><img className="marketing-blog-card-mark" src="/brand/velatra-mark.png" alt="" /><div className="marketing-blog-meta"><span>{post.category}</span><span>{post.read}</span></div><h2>{post.title}</h2><p>{post.desc}</p><div className="marketing-blog-card-bottom"><time dateTime={post.isoDate}>{post.date}</time><Link to={`/blog/${post.slug}`} aria-label={`Lire : ${post.title}`}><ArrowRight size={17} /></Link></div></article>)}</div>
        {visiblePosts.length === 0 && <p className="marketing-blog-empty">Aucun article ne correspond à votre recherche.</p>}
        <div className="marketing-blog-cta"><div><span className="marketing-kicker">UN ESPACE POUR VOTRE COACHING</span><h2>Vos programmes, votre suivi,<br />votre activité au même endroit.</h2></div><Link to="/contact?request=coach-beta" className="marketing-button marketing-button-primary">Obtenir mon accès bêta <ArrowRight size={15} /></Link></div>
      </section>
    </main>
  </>;
}
