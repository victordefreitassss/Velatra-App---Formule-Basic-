        ...
        {milestones.map((m, i) => {
          ...
          return (
            <motion.div variants={itemVariants} key={i}>
              <Card className={`flex items-center gap-6 !p-6 bg-white/60 backdrop-blur-xl transition-all shadow-sm ${!unlocked ? 'opacity-60 grayscale border-zinc-200/50' : 'border-velatra-warning/30 ring-1 ring-velatra-warning/20 hover:shadow-md hover:border-velatra-warning/50'}`}>
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-bold shadow-sm ${unlocked ? 'bg-gradient-to-br from-velatra-warning to-orange-500 text-white' : 'bg-white/50 border border-zinc-200/50 text-zinc-400'}`}>
