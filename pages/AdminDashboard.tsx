                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            ...
                            showToast(`Ajout de staff ${newStatus ? 'activé' : 'désactivé'}`, "success");
                          } catch (e) {
                            showToast("Erreur lors de la mise à jour", "error");
                          }
                        }}
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${
                          club.canAddStaff ? 'bg-blue-500 text-white shadow-sm shadow-blue-500/20' : 'text-zinc-500 hover:text-zinc-900 bg-white shadow-sm'
                        }`}
                      >
                        {club.canAddStaff ? 'Staff ON' : 'Staff OFF'}
                      </button>
                    </div>
