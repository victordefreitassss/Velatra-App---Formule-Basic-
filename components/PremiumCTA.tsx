  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] text-center px-4">
      <div className="w-20 h-20 bg-emerald-500/10 rounded-full flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(16,185,129,0.2)]">
        <LockIcon size={32} className="text-emerald-500" />
      </div>
      
      <h1 className="text-4xl md:text-5xl font-display font-bold text-zinc-900 mb-4 tracking-tight">
        Débloquez <span className="text-emerald-500">{title}</span>
      </h1>
      ...
      <div className="bg-white border border-zinc-200 rounded-3xl p-8 max-w-md w-full backdrop-blur-xl text-left mb-10">
        ...
        <ul className="space-y-4">
          {features.map((feature, idx) => (
            <li key={idx} className="flex items-start gap-3">
              <CheckCircleIcon size={20} className="text-emerald-500 shrink-0 mt-0.5" />
      ...
      <button 
        ...
        className="bg-emerald-500 hover:bg-emerald-600 text-zinc-900 px-8 py-4 rounded-2xl font-bold tracking-wide transition-all duration-300 flex items-center gap-3 shadow-[0_0_20px_rgba(16,185,129,0.4)] hover:shadow-[0_0_30px_rgba(16,185,129,0.6)] hover:scale-105"
      >
