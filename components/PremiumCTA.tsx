...
export const PremiumCTA: React.FC<PremiumCTAProps> = ({ title, description, features, paymentLink }) => {
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] text-center px-4">
      ...
      <p className="text-lg text-velatra-textMuted max-w-2xl mb-12">
        {description}
      </p>

      <div className="bg-white/5 border border-white/10 rounded-3xl p-8 max-w-md w-full backdrop-blur-xl text-left mb-10">
        ...
        <ul className="space-y-4">
          {features.map((feature, idx) => (
            <li key={idx} className="flex items-start gap-3">
              ...
              <span className="text-velatra-textMuted">{feature}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      <button 
        ...
        className="bg-velatra-accent hover:bg-velatra-accentDark text-white px-8 py-4 rounded-2xl font-bold tracking-wide transition-all duration-300 flex items-center gap-3 shadow-[0_0_20px_rgba(99,102,241,0.4)] hover:shadow-[0_0_30px_rgba(99,102,241,0.6)] hover:scale-105"
      >
